export const translations = {
  ...
  it: {
    ...
    byteBulk: {
      ...
      simpleSteps: [
        'Installa "Docker Desktop" dal sito ufficiale (gratuito) e avvialo: e il programma che fa girare i servizi sul tuo computer.',
        ...
        'Nello stesso file, alla voce "BYTEBULK_HOSTNAMES", scrivi l\'indirizzo del tuo computer sulla rete di casa (per esempio 192.168.1.10). Serve perche finisce dentro il certificato di sicurezza: un indirizzo non elencato dara errore sul telefono.',
        ...
        'Dal computer apri https://localhost:8443 e completa la procedura guidata: eta, peso, altezza, obiettivo e intolleranze. I target giornalieri vengono calcolati da soli.',
      ...
      simpleHelp: 'Il passaggio del certificato non e chiaro? E il piu delicato di tutti: scrivimi e lo vediamo insieme.',
      techIntro: 'Requisiti: Docker 24+, Docker Compose v2, openssl e circa 2 GB di RAM. Caddy fa da unico ingresso e termina il TLS; ne il database ne il backend pubblicano porte sull\'host.',
      techSteps: [
        ...
        './scripts/gen-certs.sh genera una CA privata e un certificato foglia da 397 giorni. Serve una CA e non un self-signed isolato perche Android e iOS permettono di fidarsi solo di certificati CA.',
        ...
        'Le regole nutrizionali stanno nella tabella advice_rules: aggiungerne una e una INSERT, non un rilascio. Il seeder inserisce solo le regole mancanti e non sovrascrive mai quelle modificate a mano.',
      ],
      techNotes: 'Aggiornamenti: docker compose up -d --build . Lo schema e gestito da Hibernate e i dati restano nel volume PostgreSQL; make backup produce un dump compresso prima di ogni modifica importante. Il certificato si rinnova con make certs senza dover reinstallare la CA sui dispositivi.',
      faqTitle: 'Domande frequenti',
      faqShow: 'Mostra le FAQ',
      faqHide: 'Nascondi le FAQ',
      faq: {
        ...
        tech: [
          {
            q: 'Perche un reverse proxy con HTTPS per un\'app che gira in casa?',
            a: 'Perche senza non funziona meta dell\'applicazione. Il browser espone la fotocamera e permette di installare una PWA solo in un secure context, cioe HTTPS valido o localhost. E cliccare "procedi comunque" non basta: Chrome rifiuta di registrare il Service Worker su un\'origine con errori di certificato, quindi niente offline e niente background sync.',
          },
          {
            q: 'Perche una CA privata e non un certificato self-signed?',
            a: 'Perche Android e iOS permettono di installare solo certificati CA, non certificati foglia. Con una CA la si installa una volta sola sul telefono e ogni rinnovo futuro continua a funzionare senza toccare piu nulla. Chi usa Tailscale puo saltare del tutto il passaggio: tailscale cert emette un certificato Let\'s Encrypt vero per i nomi .ts.net.',
          },
          {
            q: 'Come fa a funzionare offline senza duplicare i pasti?',
            a: 'Ogni voce nasce sul telefono con un identificativo univoco e viene scritta in IndexedDB prima di qualunque chiamata di rete. Il server ha un vincolo di unicita su quel campo, quindi rimandare la stessa voce non la duplica mai: il retry e sempre sicuro. La sincronizzazione risponde con tre liste - accettate, duplicate e rifiutate - e il dispositivo svuota la coda in base a quelle.',
          },
          {
            q: 'Perche il giorno di un pasto non viene calcolato dal server?',
            a: 'Perche "quando e successo" e "a quale giornata appartiene" sono due domande diverse. Il timestamp e un istante reale salvato in UTC, ma il giorno logico lo decide il telefono e lo invia insieme al pasto. Se lo derivasse il server, uno spuntino delle 00:30 finirebbe sulle barre di ieri per chiunque non viva sul meridiano di Greenwich.',
          },
          {
            q: 'Perche le richieste a OpenFoodFacts passano dal backend?',
            a: 'Per tre motivi: aggira i problemi di CORS, ma soprattutto da una sola cache condivisa fra tutti i dispositivi invece di una per browser, e fa si che il telefono apra connessioni verso una sola origine - condizione che rende l\'app utilizzabile dietro VPN senza split tunneling. I fallimenti di rete non vengono mai messi in cache, altrimenti un timeout momentaneo farebbe sembrare un codice a barre sconosciuto per una settimana.',
          },
          {
            q: 'I consigli nutrizionali sono scritti nel codice?',
            a: 'No, e questo e il punto. Vivono nella tabella advice_rules e il servizio che li valuta non contiene alcuna conoscenza nutrizionale: sa solo confrontare percentuali, filtrare per intolleranza e rispettare una finestra oraria. Aggiungere un consiglio e una INSERT.',
          },
        ],
        ...
        general: [
          {
            ...
            a: 'No. Gira sul tuo computer e i dati restano li. Non c\'e alcun servizio esterno che li raccoglie: le uniche richieste che escono sono quelle verso OpenFoodFacts per leggere i valori di un prodotto scansionato.',
          },
          {
            q: 'Cosa succede se registro un pasto senza connessione?',
            a: 'Viene salvato subito sul telefono e le barre di progresso si aggiornano lo stesso, con un pallino ambra accanto alla voce. Appena la rete torna, la coda si svuota da sola. La ricerca su OpenFoodFacts resta disattivata finche sei offline, ma puoi usare gli alimenti gia registrati o inserirne uno a mano.',
          },
          {
            q: 'Da dove arrivano i valori nutrizionali?',
            a: 'Da OpenFoodFacts, database aperto e collaborativo. Essendo compilato dagli utenti puo contenere errori o prodotti incompleti: per questo ogni alimento resta modificabile e l\'inserimento manuale e sempre disponibile. Ogni cibo registrato entra nella dispensa locale, cosi la volta dopo e disponibile anche offline.',
          },
          {
            q: 'Posso usarlo in definizione e non solo in massa?',
            a: 'Si. Gli obiettivi disponibili sono massa, massa pulita, mantenimento e definizione, e cambiano sia le calorie sia la ripartizione dei macro. Il calcolo mantiene comunque un minimo di grassi legato al peso corporeo, perche sotto una certa soglia non e piu una dieta ma un problema.',
          },
          {
            q: 'I numeri che mostra sono un consiglio medico?',
            a: 'No. La formula di Mifflin-St Jeor e una stima statistica, accurata sulla media ma non sulla singola persona, e i consigli sono regole generiche di buon senso alimentare. Per obiettivi seri o condizioni cliniche serve un nutrizionista, non un\'app.',
          },
          {
            q: 'Posso provarlo?',
            a: 'E un progetto personale nato per un uso mio. Scrivimi se ti interessa provarlo o semplicemente parlare delle scelte tecniche: il pattern outbox e la gestione dei fusi orari sono le parti su cui ho ragionato di piu.',
          },
        ],
      },
      reqTitle: 'Requisiti',
      ...
      httpsText: 'Fotocamera e installazione della PWA richiedono una connessione sicura. Il progetto genera una CA privata da installare una volta sul telefono; con Tailscale si puo usare un certificato Let\'s Encrypt vero e saltare il passaggio.',
      accessTitle: 'Accesso',
      contactNote: 'Progetto personale self-hosted, pensato per LAN e VPN. Scrivimi se vuoi provarlo o vedere il codice.',
      contact: 'Vuoi saperne di piu? Contattami direttamente.',
      contactTitle: 'Contatti',
      back: 'Torna alla Home',
      ctaTitle: 'ByteBulk',
